...
class ProcessPDF {
	...
	processPages(page, cols) {
		...
		page.forEach((mark, markIndex) => {
			...
			mark.R.forEach((cell) => {
				...
				let isEventNumber = place === 1 && cellText.match(/[A-Z]/) === null;
				...
				if (!initialRow && labelsSet && updateTime && !updatedCell) {
					...
					let nextRowStartsNext = nextPage && this.cleanText(nextPage.R[0].T).match(/[0-9]{2}-/) !== null;

					if (isEventNumber || // event number is always in two parts
						...
						place--;
						row[labels[place]] += (isEventNumber ? '' : ' ') + cellText;

						// if we were on 6 and we just added to the 6th columns data
						// we need to re-increment in order to ensure the event_number prefix isn't dropped
						if (place > 2 && nextRowStartsNext) {
							place++;
						}
					} else {
	...
	isDateTime(text) {
		...
		return text.match(/[0-9]{4}-[0-9]{2}-[0-9]{2} [0-9]{2}:[0-9]{2}:[0-9]{2}/) === null ? false : true;
	}
}
